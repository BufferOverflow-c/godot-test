...
void Player::updatePosition(const double delta) {
  ...
  position = Vector2(Math::clamp(position.x, 0.f + hitBox.x/2, screenSize.x - hitBox.x/2), Math::clamp(position.y, 0.f + hitBox.y/2, screenSize.y - hitBox.y/2));
  set_position(position);
}
...
void Player::_bind_methods() {
  ...
  ADD_SIGNAL(MethodInfo("hit_event", PropertyInfo(Variant::OBJECT, "Area2D"), PropertyInfo(Variant::NODE_PATH, "Node2D")));  
